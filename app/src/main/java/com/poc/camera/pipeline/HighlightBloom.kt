 ...
 *
 * The lift is a smooth multiplicative gain ramped in by proximity to per-channel clipping: a
 * pixel whose brightest channel is below [BokehParams.highlightThreshold] is untouched (gain 1),
 * a pixel clipped in any channel gets the full [BokehParams.highlightBoost], and the ramp
 * between is a smoothstep so no hard edge is introduced. Gating on the max channel (rather than
 * luma) means a highlight clipped in only one channel -- a coloured LED or string light -- blooms
 * like a neutral one, as a real lens does; a neutral highlight is unchanged (max == luma). Working directly on the gamma-encoded channels with float headroom is a
    ...
    fun apply(
        ...
        PipelineParallel.parallelRows(size, chunkCount) { start, end ->
            for (i in start until end) {
                ...
                // Gate on proximity to per-channel clipping, not luma: a highlight clipped in
                // only one channel (a coloured LED / string light) is a genuine clipped
                // highlight even though its luma is low, and a real lens blooms it as strongly
                // as a neutral one. Using the max channel lets those coloured highlights bloom;
                // a neutral highlight (max == luma) is unchanged from the previous luma gate.
                val clip = maxOf(r, g, b).toDouble()
                // Smooth ramp from threshold (0) to fully clipped 255 (1), scaled to background.
                val ramp = smoothstep(threshold, 255.0, clip) * backgroundness[i]
