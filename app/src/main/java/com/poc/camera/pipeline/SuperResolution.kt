 ...
 * [SCALE] * (fx - offX, fy - offY). The reference itself has a zero offset field, so its
 * pixels land exactly on the even output texels. (The task states this as
 * 2*(x + dxAt): our dxAt is -offX, the frame-to-reference displacement.)
 *
 * ## Ghost gating
 *
 ...
 * ([GHOST_BLUR_RADIUS] box blur): genuine super-Nyquist detail -- which the reference
...
object SuperResolution {
    ...
    /** Box-blur radius of the luma low-pass the ghost gate compares on. */
    private const val GHOST_BLUR_RADIUS = 2

    // Box-blur radius of the luma the TILE aligner matches on. The sub-pixel offset is a
    // property of the smooth, band-limited scene structure; aliased super-Nyquist detail
    // (which shifts unpredictably frame to frame) would otherwise dominate the per-tile MAD
    // and corrupt the very sub-pixel estimate SR depends on. Low-pass matching locks onto
    // the base; the full-resolution samples are still splatted, so no detail is lost.
    private const val ALIGN_BLUR_RADIUS = 2
    ...
    fun superResolve(frames: List<Frame>): Result {
        ...
        // sub-pixel estimate (see [ALIGN_BLUR_RADIUS]).
    ...
    private fun splatFrame(
        ...
        for (fy in 0 until height) {
            val rowBase = fy * width
            for (fx in 0 until width) {
                val offX = offsets.offsetXAt(fx, fy)
                val offY = offsets.offsetYAt(fx, fy)
                val refX = fx - offX
                val refY = fy - offY
    ...
    private fun blurredLuma(frame: Frame, precomputed: LumaPlane?): DoubleArray {
        ...
        return BoxBlur.blur(src, frame.width, frame.height, GHOST_BLUR_RADIUS)
    }

    /** Box-blurred luma plane of [frame] used for tile alignment (see [ALIGN_BLUR_RADIUS]). */
    private fun blurLumaPlane(frame: Frame, luma: LumaPlane): LumaPlane {
        val src = DoubleArray(luma.values.size) { luma.values[it].toDouble() }
        val blurred = BoxBlur.blur(src, frame.width, frame.height, ALIGN_BLUR_RADIUS)
