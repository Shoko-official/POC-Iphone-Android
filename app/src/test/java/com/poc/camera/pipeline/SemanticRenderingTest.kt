 ...
 * smoothing reduces chroma variance, the strength endpoints/blend behave, zero masks are a
 * bit-exact passthrough, and the parallel path is bit-identical to the serial one.
 */
class SemanticRenderingTest {
    ...
    fun skyDeepeningIsBoundedToTwelveCodesAndPreservesHue() {
        ...
        val sky = ones(frame)
        val fol = zeros(frame)
        val out = SemanticRendering.apply(frame, sky, fol, params)
    ...
    fun skySmoothingReducesChromaVariance() {
        ...
        val out = SemanticRendering.apply(frame, sky, fol, smoothOnly)
    ...
    fun strengthZeroIsBitExactPassthrough() {
        ...
        val out = SemanticRendering.apply(frame, ones(frame), zeros(frame), params.copy(strength = 0.0))
    ...
    fun strengthBlendDeepensLessThanFull() {
        ...
        val half = chromaMag(SemanticRendering.apply(frame, sky, fol, params.copy(strength = 0.5)), 8, 8)
        val full = chromaMag(SemanticRendering.apply(frame, sky, fol, params.copy(strength = 1.0)), 8, 8)
    ...
    fun zeroMasksLeaveRgbUnchanged() {
        ...
        val out = SemanticRendering.apply(frame, zeros(frame), zeros(frame), params)
    ...
    fun outputIsAlwaysOpaque() {
        ...
        val out = SemanticRendering.apply(frame, ones(frame), zeros(frame), params)
    ...
    fun parallelMatchesSerial() {
        ...
        val serial = SemanticRendering.apply(frame, sky, fol, params, PipelineParallel.SERIAL_CHUNKS)
        val parallel = SemanticRendering.apply(frame, sky, fol, params, PipelineParallel.parallelism)
